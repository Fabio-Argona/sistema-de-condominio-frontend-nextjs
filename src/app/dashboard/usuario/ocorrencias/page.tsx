...
export default function UsuarioOcorrenciasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const created = await post(`/ocorrencias/usuario/${user.id}`, payload) as Ocorrencia;
    if (created) {
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Nova Ocorrência">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea value={formData.descricao} onChange={(e) => setFormData({ ...formData, descricao: e.target.value })} required rows={4} placeholder="Descreva detalhadamente o problema..." className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all" />
          </div>
